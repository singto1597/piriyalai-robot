...
### 🛠️ Robot_High_Automation - factor เผลอ 0 เกิน → U-turn หมุน 5-10 รอบ (ดูเหมือน "มอเตอร์ตัวหนึ่งพัง")
- **Context/Problem:** หุ่นเลี้ยว/กลับตัว แล้วมอเตอร์ตัวหนึ่ง "หยุดนิ่ง" แต่ยกหุ่นขึ้น (ไร้โหลด) หมุนปกติ เปลี่ยนมอเตอร์แล้วยังเดิม และเดินตรงปกติ — ดูผิวเผินเหมือนฮาร์ดแวร์พัง
- **Root Cause:** `TURN_180_MS_FACTOR` เผลอเป็น 360000 (ต้องเป็น 36000) → U-turn = 360000/turnSpeed = 3600-7200ms (5-10 รอบเต็มกำลังต่อเนื่อง) → โหลดกระแสสูงมากนานมาก → ช่องไดรเวอร์/สาย/ขั้วมอเตอร์ที่อ่อนหมุนไม่ไหว (หยุดนิ่ง) พอไร้โหลดกระแสลดกลับหมุนปกติ = จุดอ่อนฮาร์ดแวร์ถูก "เปิดโปง" ด้วยเวลาหมุนยาวเกิน ไม่ใช่บั๊กลอจิกมอเตอร์ (โค้ดทุกคำสั่งหมุน SL/SR/pivot/align สั่งครบ 4 ล้อ ไม่เคยสั่งตัวเดียวให้หยุด)
- **Correct Pattern/Solution:** เวลาเจอ "มอเตอร์หมุนไม่ไหวเฉพาะตอนเลี้ยว (ไร้โหลดปกติ)": (1) เช็คเวลาเลี้ยวใน config ก่อนเสมอ — factor÷turnSpeed ต้องเป็นหลักร้อย ms (90°≈240, 180°≈360) ถ้าขึ้นหลักพัน = เผลอ 0 เกิน (2) ตรวจความสมมาตรโค้ดว่าหมุนครบ 4 ล้อ (3) ถ้าแก้เวลาแล้วยังเป็น → ฮาร์ดแวร์ช่องนั้นจริง ย้ายขั้วมอเตอร์ไปช่องอื่นทดสอบเพื่อแยก ไดรเวอร์ vs มอเตอร์
- **Date Added:** 2026-08-22
